...
def kernel_permutation_test(data_train,data_test,X_ker,Y_ker,weights_model,test_stat="DATE",n_bins=10,n_permutations=100,reg=1):
    ...
    if test_stat == "DATE":
        W0 = get_W_matrix(X_ker(data_train.X0).evaluate(),reg)
        W1 = get_W_matrix(Y_ker(data_train.X1).evaluate(),reg)
    ...
    if test_stat == "DATE":
        W0_permuted = get_W_matrix(X_ker(permuted_train_data.X0).evaluate(),reg)
        W1_permuted = get_W_matrix(Y_ker(permuted_train_data.X1).evaluate(),reg)
    ...
    for i in range(n_permutations):
        ...
        if test_stat == "DATE":
            permuted_stats.append(DATE_test_stat(permuted_train_data,permuted_test_data,X_ker,Y_ker,weights_test,W0_permuted,W1_permuted))
        
        elif test_stat == "DETT":
            permuted_stats.append(DETT_test_stat(data_train,permuted_test_data,X_ker,Y_ker,weights_test,W1_permuted))
